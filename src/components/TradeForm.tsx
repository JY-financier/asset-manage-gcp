...
interface TradeFormProps {
    tradeId?: number; // 있으면 수정(PATCH), 없으면 신규(POST)
    initial?: Partial<TradeFormInitial>;
}
...
export default function TradeForm({ tradeId, initial }: TradeFormProps) {
    const router = useRouter();
    const isEdit = tradeId !== undefined;

    const [ticker, setTicker] = useState(initial?.ticker ?? "");
    const [name, setName] = useState(initial?.name ?? "");
    const [category, setCategory] = useState(initial?.category ?? "주식(국내)");
    const [account, setAccount] = useState(initial?.account ?? "");
    const [tradeDate, setTradeDate] = useState(initial?.trade_date ?? todayStr());
